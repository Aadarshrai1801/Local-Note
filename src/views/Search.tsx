 ...
 * carries its position in the conversation rather than just a score.
 */
...
export function Search(): ReactNode {
  ...
  const results = useAsyncData(
    () =>
      trimmed.length === 0
        ...
        : api.search({ query: trimmed, semantic, keyword, sinceDays: sinceDays ?? undefined, limit: 60 }),
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-8 py-9 lg:px-12">
      ...
      <p className="mt-2 max-w-[66ch] text-[13.5px] leading-relaxed text-ink-400">
        Keyword search matches exact words across every transcript. Semantic search adds
        paraphrases and needs the local embedding model, but keyword results always work without it.
      </p>
      ...
      >
        <div className="relative">
          ...
          <input
            ...
            autoFocus
            className="field h-11 pl-10 pr-24 text-[14px]"
          />
          <span className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 font-mono text-[10px] uppercase tracking-[0.1em] text-ink-600">
            {trimmed.length > 0 ? plural(hits.length, 'hit') : 'type to search'}
          </span>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <label
            className={cx(
              ...
              llmMissing ? 'text-ink-600' : 'text-ink-300'
            )}
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.08em] text-ink-500 transition-colors hover:text-ink-200 disabled:opacity-40"
          >
            <Icon name={reindexing ? 'refresh' : 'database'} size={12} className={cx(reindexing && 'animate-spin')} />
      ...
      <div className="mt-7 grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <section aria-labelledby="results-heading" className="min-w-0">
          ...
          {trimmed.length === 0 ? (
            ...
            <LoadingBlock label="Searching local transcripts…" />
          ) : results.error ? (
        ...
        {/* Ask across meetings -------------------------------------- */}
        <aside className="rounded-md border border-signal-500/25 bg-signal-500/[0.04]">
                    ...
                    {answer.citations.map((citation, index) => (
                      <li key={`${citation.meetingId}-${citation.segmentId}`}>
                        <button
                          ...
                          className="group flex w-full items-start gap-2 rounded px-1 py-1 text-left transition-colors hover:bg-signal-500/10"
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span className="mt-0.5 block font-mono text-[10px] text-ink-500">
                              at {formatClock(citation.startMs)} ·{' '}
                              {truncate(citation.text, 64)}
                            </span>
                          </span>
                          <Icon
                            name="arrowUpRight"
                            size={12}
                            className="mt-1 shrink-0 text-ink-700 group-hover:text-signal-300"
                          />
                        </button>
                      </li>
                    ))}
      ...
      {isMockApi && (
        <p className="mt-10 border-t border-ink-800 pt-4 font-mono text-[10px] uppercase tracking-[0.1em] text-ink-600">
...
function MeetingGroupCard({ group, query }: { group: MeetingGroup; query: string }): ReactNode {
  ...
  return (
    <article className="rounded-md border border-ink-800 bg-ink-900/40">
      <button
        type="button"
        onClick={() => openMeeting(group.meetingId)}
        className="group flex w-full items-baseline gap-3 px-3.5 py-2.5 text-left"
      >
        <Icon name="file" size={14} className="shrink-0 translate-y-0.5 text-ink-500" />
        <span className="min-w-0 flex-1">
          <span className="block truncate text-[14px] font-medium tracking-[-0.01em] text-ink-100 group-hover:text-white">
            {group.title}
          </span>
        ...
        <span className="chip shrink-0">best · {viaLabel(best.via)}</span>
        <Icon
          name="chevronRight"
          size={14}
      ...
      <div className="relative mx-3.5 h-9 border-t border-ink-800/70">
        <span
          aria-hidden="true"
          className="absolute left-0 right-0 top-[18px] h-px bg-ink-800"
        />
        <span className="absolute left-0 top-[22px] font-mono text-[9px] uppercase tracking-[0.08em] text-ink-700">
          0:00
        </span>
        {group.hits.map((hit) => (
          <span
            ...
            className={cx(
              ...
              hit.via === 'keyword' ? 'bg-ink-200' : 'bg-signal-400'
            )}
            style={{ left: `${Math.min(98, Math.max(0, ((hit.startMs ?? 0) / span) * 100))}%` }}
          />
        ))}
        <span className="absolute right-0 top-[22px] font-mono text-[9px] uppercase tracking-[0.08em] text-ink-700">
        ...
        {group.hits.slice(0, 6).map((hit) => (
          <li key={`${hit.segmentId ?? 'hit'}-${hit.via}-${hit.startMs ?? 0}-${hit.score}`}>
            <button
              ...
              className="flex w-full items-start gap-3 px-3.5 py-2.5 text-left transition-colors hover:bg-ink-900/70"
            >
              <span className="w-12 shrink-0 pt-0.5 text-right font-mono text-[11px] tabular-nums text-ink-500">
                {formatDuration(hit.startMs)}
              </span>
              <span className="min-w-0 flex-1">
                <span className="flex items-baseline gap-2">
                  <span className="shrink-0 text-[11px] font-semibold uppercase tracking-[0.1em] text-ink-400">
                    {hit.speakerLabel ?? 'Unknown'}
                  </span>
                  <span className="chip shrink-0">{viaLabel(hit.via)}</span>
                </span>
                <span className="mt-1 block max-w-[70ch] text-[13px] leading-relaxed text-ink-200">
      ...
      {group.hits.length > 6 && (
        <p className="border-t border-ink-800/60 px-3.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.08em] text-ink-600">
